...
<img align="right" src="./media/normalized_triangle_sampler.jpg" width="450" style="padding-left:20px">

Points in a trianlge can be naivly generated by $f(\alpha, \beta) = \beta \bar v + \alpha \beta (\bar u - \bar v)$, where $\alpha$, $\beta$ are sampled uniformly, and $\bar u$, $\bar b$ are vectors representing two of the sides of the triangle. Putting up the equation and assuming that $\alpha ' = \alpha$, gives:

$$
\begin{align*}
  \beta'\det( [u,v]) d\beta ' =& \frac{\det([u,v])}{2 \cdot 1} d\beta &\Leftrightarrow & \quad \\ 
  \frac{\beta'^2}{2} = & \frac{1}{2}\beta +C &\Leftrightarrow & \quad \\
  \beta'= & \sqrt{\beta +C} &\Leftrightarrow & \quad \\
  \bigg /  \beta(0) = 0 &\Rightarrow C = 0 / \bigg / & \Leftrightarrow &\\
  \beta' = &\sqrt{\beta} &\Rightarrow & \quad \\
  \\
  h(\alpha, \beta) = f(\alpha', \beta')=& \sqrt{\beta} \bar v + \alpha  \sqrt{\beta} (\bar u - \bar v) & & \quad 
\end{align*}
$$
